...
const SpotifyArtistSearch = () => {
  ...
  function track_info(track){
    //getTrackInfo(track.id, token).then(console.log("hi"))
    return (
    console.log("hi\n"), 
    ...
    <p className="text-sm text-gray-600">
      Album: {track.id.album} • Popularity: {track.popularity}
    </p>
  </div>
    )
  }
  const getTrackInfo = async (songid, accessToken) => {
    try {
      const topTracksUrl = `https://api.spotify.com/v1/audio-features/${songid}`;
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              topTracks.map(
                ...
                (track) => ( //map function is used for iterating through elements of an array and displaying them in this case
                track_info(track)
              ))
              }
            </div>
